...
void frdm_client::value_updated(M2MBase *base, M2MBase::BaseType type)
{
    printf("PUT operation called \n");
    ...
    if(base) {
        if(base->base_type() == M2MBase::Resource) {
            ...
            printf("PUT operation payload is %s\n",value.c_str());
                
            bpmPutValue = atoi(value.c_str());
